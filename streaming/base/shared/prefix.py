...
def _check_and_find_retrying(streams_local: list[str], streams_remote: list[Union[str, None]],
    ...
    for _ in range(1 + retry):
        try:
            return _check_and_find(current_streams_local, streams_remote, shm_name)
        except Exception as err:
            errs.append(err)
            # Create new unique directories for retry
            current_streams_local = _make_unique_local_dirs(current_streams_local, [])
            sleep(TICK)
    
    # If all retries failed, return the last attempt result
    return _check_and_find(current_streams_local, streams_remote, shm_name)
